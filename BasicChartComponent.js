...
export default function ChartComponent({ records, groupBy, countBy, stackBy, bucketBy }) {
    ...
    // Prepare datasets with distinct colors for each stack key
    const datasets = stackKeys.map((key, index) => ({
        label: key,
        data: labels.map(label => aggregatedData[label][key] || 0),
        backgroundColor: getStatusColor(key, index, stackKeys.length),
    }));
...
// Updated function to ensure distinct colors for each stack key
function getStatusColor(key, index, totalCategories) {
    const colorPalette = [
